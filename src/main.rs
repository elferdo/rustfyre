...
fn main() -> Result<(), Report<AppError>> {
    ...
    let mut array = Array2D::filled_with(0u32, 2160, 3840);

    let mut coords = Vec::with_capacity(1000000000);
    ...
    let xmin = coords.iter().fold(0.0f64, |b, s| b.min(s.x));
    let xmax = coords.iter().fold(0.0f64, |b, s| b.max(s.x));
    let ymin = coords.iter().fold(0.0f64, |b, s| b.min(s.y));
    let ymax = coords.iter().fold(0.0f64, |b, s| b.max(s.y));

    for c in coords {
        let x4k = ((c.x - xmin) * 3839.0 / (xmax - xmin)) as usize;
        let y4k = ((c.y - ymin) * 2159.0 / (ymax - ymin)) as usize;
    ...
    let v2: Vec<_> = v.into_iter().map(|v| (v * 255.0 / max_v) as u8).collect();
